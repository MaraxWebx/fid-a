...
def require_center_owner(center_id: str, current_user: dict):
    object_id = parse_object_id(center_id, "center id")
    if current_user.get("center_id") != str(object_id):
        raise HTTPException(status_code=403, detail="Center ownership required.")
    return object_id


def require_client_subject(current_user: dict):
    return parse_object_id(current_user["sub"], "user id")


def build_center_auth_response(center: dict):
    ...
    return {
        ...
        "user": serialize_center(center),
        "activation": build_activation_status(center),
    }


def build_client_auth_response(user: dict):
    center_id = serialize_id(user.get("center_id")) if user.get("center_id") else None
    return {
        "access_token": create_access_token(
...
def delete_uploaded_asset(storage_path: str | None):
    if not storage_path:
        return
    try:
...
def validate_booking_actor(
    ...
    *,
    ...
    if role and role != token_role:
        raise HTTPException(status_code=403, detail="Token role does not match requested role.")

    if token_role == "client":
        user_id = require_client_subject(current_user)
        # TODO: remove user_email argument after older clients are retired.
        user = db.users.find_one({"_id": user_id, "role": "client"})
    ...
    if token_role == "center":
        # TODO: remove center_id argument after older clients are retired.
        center_object_id = parse_object_id(current_user["center_id"], "center id")
        if center_id and center_id != str(center_object_id):
            raise HTTPException(status_code=403, detail="Center ownership required.")
...
def register_center(payload: CenterRegistrationPayload, request: Request):
    ...
    auth_response = build_center_auth_response(created_center)
...
def activate_center_subscription(center_id: str, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def login_center(payload: LoginPayload):
    ...
    if password_needs_rehash(center.get("password_hash")):
        db.centers.update_one(
            {"_id": center["_id"]},
            {"$set": {"password_hash": hash_password(payload.password), "updated_at": datetime.now(UTC)}},
        )
        center = db.centers.find_one({"_id": center["_id"]})

    return build_center_auth_response(center)
...
def register_client(payload: ClientRegistrationPayload):
    ...
    return build_client_auth_response(user)
...
def create_center_membership(payload: CenterAssociationPayload, current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    user = db.users.find_one({"_id": user_id, "role": "client"})
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")

    # TODO: remove payload.email from mobile contract after older clients are retired.
    center = find_center_by_invitation(payload.invitation_code)
    add_client_center_membership(user["_id"], center["_id"], source="invitation")
...
def get_center_activation_status(center_id: str, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
...
def update_center_onboarding(center_id: str, payload: CenterOnboardingPayload, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    ...
    if payload.logo_url is not None:
        if payload.logo_url.strip():
            raise HTTPException(status_code=400, detail={"code": "LOGO_UPLOAD_REQUIRED", "message": "Carica il logo tramite upload immagine."})
    ...
    if clear_logo:
        delete_uploaded_asset(center.get("logoStoragePath"))
        update_fields["logoStoragePath"] = None
...
def update_center_profile(center_id: str, payload: CenterProfilePayload, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
...
def update_center_availability(center_id: str, payload: CenterAvailabilityPayload, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
...
async def upload_center_logo(request: Request, center_id: str, file: UploadFile = File(...), current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def update_center_services(center_id: str, payload: CenterServicesCatalogPayload, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
...
def get_profile(email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    document = db.users.find_one({"_id": user_id, "role": "client"})
...
def update_user_profile(payload: UserProfilePayload, email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    document = db.users.find_one({"_id": user_id, "role": "client"})
...
def get_user_bookings(email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    user = db.users.find_one({"_id": user_id, "role": "client"})
...
def get_user_stats(email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    user = db.users.find_one({"_id": user_id, "role": "client"})
...
def get_user_favorite_centers(email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    user = db.users.find_one({"_id": user_id, "role": "client"})
...
def get_user_center_memberships(email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    user = db.users.find_one({"_id": user_id, "role": "client"})
...
def toggle_user_favorite_center(center_id: str, email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_client)):
    user_id = require_client_subject(current_user)
    # TODO: remove email query fallback from mobile contract after older clients are retired.
    user = db.users.find_one({"_id": user_id, "role": "client"})
...
def get_notifications(
    ...
    if role == "client":
        ...
        if center_id:
            query["center_id"] = require_center_owner(center_id, current_user)
        else:
            query["center_id"] = parse_object_id(current_user["center_id"], "center id")
    else:
...
def mark_notifications_read(payload: NotificationReadPayload, current_user: dict = Depends(get_current_user)):
    object_ids = [parse_object_id(value, "notification id") for value in payload.notification_ids]
    if not object_ids:
        return {"updated": 0}

    ownership_query: dict = {}
    if current_user["role"] == "client":
        ownership_query = {"user_id": require_client_subject(current_user)}
    elif current_user["role"] == "center":
        ownership_query = {"center_id": parse_object_id(current_user["center_id"], "center id")}

    result = db.notifications.update_many(
        {"_id": {"$in": object_ids}, **ownership_query},
        {"$set": {"is_read": True, "updated_at": datetime.now(UTC)}},
    )
    return {"updated": result.modified_count}


def get_center_business_insights(center_id: str, period: str = Query(default="month"), current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def export_center_business_report(center_id: str, period: str = Query(default="month"), current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def export_center_no_show_report(center_id: str, period: str = Query(default="month"), current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def delete_center_monthly_no_show_report(center_id: str, period: str = Query(default="month"), current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def get_center_dashboard(center_id: str, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def get_center_clients(center_id: str, current_user: dict = Depends(get_current_center)):
    object_id = require_center_owner(center_id, current_user)
...
def get_center_client_detail(center_id: str, client_id: str, current_user: dict = Depends(get_current_center)):
    center_object_id = require_center_owner(center_id, current_user)
...
def get_center_bookings(center_id: str, date: str | None = Query(default=None), current_user: dict = Depends(get_current_center)):
    center_object_id = require_center_owner(center_id, current_user)
...
def get_center_user_stats(center_id: str, email: str = Query(default=DEFAULT_PROFILE_EMAIL), current_user: dict = Depends(get_current_center)):
    center_object_id = require_center_owner(center_id, current_user)
    center = db.centers.find_one({"_id": center_object_id})
    if not center:
        raise HTTPException(status_code=404, detail="Center not found.")
...
def get_center_booking_detail(center_id: str, booking_id: str, current_user: dict = Depends(get_current_center)):
    center_object_id = require_center_owner(center_id, current_user)
...
def create_review(payload: ReviewPayload, current_user: dict = Depends(get_current_client)):
    booking_object_id = parse_object_id(payload.booking_id, "booking id")
    user_id = require_client_subject(current_user)
    # TODO: remove payload.user_email from mobile contract after older clients are retired.
    user = db.users.find_one({"_id": user_id, "role": "client"})
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")
...
def create_booking(payload: BookingPayload, current_user: dict = Depends(get_current_client)):
...
def update_booking(booking_id: str, payload: BookingUpdatePayload, current_user: dict = Depends(get_current_center)):
    booking_object_id = parse_object_id(booking_id, "booking id")
...
def update_booking_status(booking_id: str, payload: BookingStatusPayload, current_user: dict = Depends(get_current_center)):
    booking_object_id = parse_object_id(booking_id, "booking id")
    booking = db.bookings.find_one({"_id": booking_object_id})
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found.")
...
def cancel_booking(
    ...
    if actor_role == "client" and is_past_booking(booking):
